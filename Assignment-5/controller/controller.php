...
class controller extends model
{
    public function __construct()
    {
       ...
       parent::__construct();
        ...
        // show coursecategory list data
         $shwcourse=$this->showdata('tbl_coursecategory');
    
         // show coursecategory list data
        $shwcourselist=$this->showdata('tbl_courselist');

        //show category and it's product
        if(isset($_GET["coursedetail"]))
        {
            $id=$_GET["coursedetail"];
            $shwlist=$this->selectdetail('tbl_courselist','coursecategory_id',$id);
        }
    ...
    if(isset($_POST["submit"]))
    {
        ...
        try 
        {
            ...
            //Recipients
            $mail->setFrom('jigna434@gmail.com', 'Mail sending');
            ...
            $mail->Subject = 'Thnaks for create a account';
            // $chk=$this->forgetpassword('t_student','email',$em);
            $mail->Body="Welcome,".$email."to create a account with us";
            $mail->send();
    ...
    // delete data 
        if(isset($_GET["deldata"]))
        {
            $delid=$_GET["deldata"];
            $id=array("s_id"=>$delid);
            $chk=$this->deletedata('t_student',$id);
            if($chk)
            {
                unset($_SESSION["s_id"]);
                unset($_SESSION["email"]);
